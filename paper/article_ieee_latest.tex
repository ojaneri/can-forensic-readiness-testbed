...
\title{A Reproducible Software-Defined CAN Testbed for Automotive Cybersecurity and Forensic-Readiness Analysis}
...
\begin{abstract}
Controller Area Network (CAN) remains common in production and legacy vehicles, but it does not provide native authentication, confidentiality, or sender verification. This paper presents a reproducible, software-defined CAN testbed for automotive cybersecurity and forensic-readiness analysis. The artifact packages labeled internal experiments, a 10-run intensity matrix, external ICSim trace ingestion, didactic spoof-packet injection, a short wall-clock timing capture, normalized datasets, source metadata, SHA-256 manifests, validation scripts, and observer-safe browser demos. Across internal labeled experiments, the testbed generated 477,738 CAN frames. External-source compatibility is demonstrated by importing 6,158 frames from the public ICSim \texttt{sample-can.log} trace, while a visual injection package adds 45 labeled spoofed speed, door, and turn-signal packets. A wall-clock package records 1,167 software-only frames over 20 seconds with no negative inter-arrival values. A rule-based triage heuristic is retained only as a label-consistency and forensic-screening sanity check, not as a real-world intrusion detection benchmark. The contribution is an evidence-preserving reproducibility workflow rather than a claim of physical CAN fidelity or operational IDS performance.
\end{abstract}

\begin{IEEEkeywords}
automotive cybersecurity, CAN bus, forensic readiness, reproducible research, testbed, ICSim, virtual CAN
\end{IEEEkeywords}
...
This paper addresses that gap with a reproducible software-defined CAN testbed designed for automotive cybersecurity and forensic-readiness analysis. The goal is explicitly bounded: the artifact does not claim physical CAN fidelity, production-vehicle realism, or operational intrusion-detection performance. Instead, it provides a reproducible workflow for generating and preserving traces, importing external traffic, injecting didactic events, validating hashes, and replaying evidence safely in a browser.
...
\begin{table}[t]
\caption{Artifact Reproducibility Comparison}
\label{tab:artifactcompare}
\centering
\footnotesize
\begin{tabular}{lcccc}
\toprule
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../analysis/figures/reproducible_evidence_pipeline_v5.pdf}
\caption{Reproducible CAN evidence pipeline. Internal runs, external traces, and wall-clock capture feed a common normalization and evidence-packaging workflow with manifests, DOI-backed artifacts, and observer-safe demos.}
\label{fig:pipeline}
\end{figure}
...
\subsection{Ethical Scope and Safety Boundary}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.95\textwidth]{../analysis/figures/evidence_coverage_dashboard_v5.pdf}
\caption{Evidence coverage dashboard for the v5 artifact. The figure summarizes internal phase balance, external and injected packages, didactic speed contrast, and artifact-review posture.}
...
\subsection{Wall-clock Timing Capture}
Experiment 010 addresses a narrow reproducibility concern left by deterministic fast-run experiments. It generated 1,167 software-only CAN-like frames over 20.000022 seconds using wall-clock sleeps and monotonic timestamps. The package records four arbitration IDs, mean inter-arrival time of 0.017153 seconds, median inter-arrival time of 0.009923 seconds, and zero negative inter-arrival values. This does not validate physical CAN arbitration or electrical behavior; it demonstrates that the evidence workflow can preserve real elapsed timing when experiments are not fast-run.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{../analysis/figures/exp010_wallclock_timing_v5.pdf}
\caption{Experiment 010 wall-clock timing evidence: frame timeline and inter-arrival distribution from the software-only real-time capture.}
\label{fig:exp010-timing}
\end{figure}
...
RQ2 & Controlled spoofing/flooding variation is supported. & Exp. 006/007 plus Exp. 010; 477,738 internal frames; 10-run intensity matrix; 20s wall-clock capture. & Synthetic labels and fast-run timing. \\
...
The current artifact is software-defined and observer-safe. It does not claim physical CAN fidelity, production-vehicle attack realism, or operational IDS performance. Future work includes SocketCAN/\texttt{vcan} capture with kernel interfaces, ROAD or can-train-and-test import into the same schema, larger multi-run wall-clock timing campaigns, and HIL validation with documented legacy ECUs.

\section{Conclusion}
This paper presented a reproducible software-defined CAN testbed for automotive cybersecurity and forensic-readiness analysis. The artifact generates labeled internal traces, expands them through a 10-run intensity matrix, imports an external ICSim trace, supports didactic spoof injection, preserves evidence through manifests and hashes, and provides observer-safe browser demos. The v5 framing avoids overclaiming detection or physical realism and emphasizes reproducible, low-cost, evidence-aware learning and review.
